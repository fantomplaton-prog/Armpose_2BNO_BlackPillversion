...
def parse_ble_line(line: str) -> tuple[Quat, Quat] | None:
    ...
    на груди - отсюда arm=parts[0:4], chest=parts[4:8]. (Пробовали убрать
    своп после переезда на F401/новую плату - не помогло, проблема не
    в этом, поэтому своп возвращён.)
